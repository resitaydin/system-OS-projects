...
void *worker_thread(void *arg) {
    ...
    while (1) {
        ...
        while (file_buffer.count == 0 && !file_buffer.done && !stop) {
            printf("Worker thread %d waiting (buffer empty).\n", thread_id);
        ...
        if (file_buffer.count == 0 && (file_buffer.done || stop)) {
            ...
            printf("Worker thread %d exiting (no more work).\n", thread_id);
            break;
        }
        ...
        printf("Worker thread %d processing file %s.\n", thread_id, file_info.src_name);
        ...
        printf("Worker thread %d finished processing file %s.\n", thread_id, file_info.src_name);
    }

    // Wait at the barrier to ensure all threads reach this point before finishing
    printf("Worker thread %d waiting at barrier.\n", thread_id);
    pthread_barrier_wait(&barrier);
    printf("Worker thread %d passed the barrier.\n", thread_id);
    ...
    while ((entry = readdir(dir)) != NULL && !stop) {
        ...
        if (S_ISREG(statbuf.st_mode)) {
            ...
            while (file_buffer.count == buffer_size && !stop) {
                printf("Manager waiting (buffer full).\n");
            ...
            printf("Manager added file %s to buffer.\n", src_path);
...
void *manager_thread(void *arg) {
    ...
    printf("Manager thread finished processing directories.\n");
...
int main(int argc, char *argv[]) {
    ...
    pthread_barrier_init(&barrier, NULL, num_workers); // Initialize barrier for workers only
