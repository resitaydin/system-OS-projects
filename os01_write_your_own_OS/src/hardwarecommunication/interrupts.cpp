...
common::uint32_t InterruptHandler::sys_getpid(){
    return interruptManager->taskManager->getCurrentTaskPid();
}
...
common::uint32_t InterruptHandler::sys_execve(common::uint32_t entrypoint){
    return interruptManager->taskManager->execveTask((void (*)()) entrypoint);
}

bool InterruptHandler::sys_waitpid(common::uint32_t esp){
    return interruptManager->taskManager->waitpidTask(esp);
}
